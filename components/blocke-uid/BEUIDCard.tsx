...
const getCost = (digits: number) => {
  if (digits < 4) return 20000;
  if (digits <= 5) return 15000;
  if (digits <= 7) return 8000;
  if (digits <= 10) return 4000;
  return 500;
};

const cost = getCost(digits);
    ...
    <div className="relative bg-gradient-to-br from-[#4F46E5] via-[#7C3AED] to-[#9333EA] p-[2px] rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300 dark:from-gray-700 dark:via-gray-800 dark:to-gray-900">
      <div className={`bg-white dark:bg-gray-800 rounded-xl p-6 backdrop-blur-sm relative ${theme === 'dark' ? 'dark' : ''}`}> {/* Apply dark class conditionally */}
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-5 gap-2"> {/* Updated grid to 5 columns */}
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-2 rounded-lg text-center">
              <label className="text-xs text-gray-500 dark:text-gray-300">Fee</label>
              <p className="font-semibold text-gray-900 dark:text-white">{cost} BE</p>
            </div>
            {mintedAt && (
              <div className="bg-gray-50 dark:bg-gray-700 p-2 rounded-lg text-center col-span-3"> {/* Spans 3 columns */}
