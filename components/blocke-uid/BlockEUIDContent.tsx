...
import TransactionStatus from "../TransactionStatus" // Import TransactionStatus
...
export default function BlockEUIDContent({ hasUID, onUIDsFetched }: BlockEUIDContentProps) {
  ...
  const handleMint = async () => {
    ...
    setError("")
    setTransactionState("pending") // Set transaction state to pending
    ...
    try {
      ...
      if (isMinted) {
        ...
        setTransactionState("error") // Set transaction state to error
      ...
      setId("")
      setTransactionState("success") // Set transaction state to success

      setShowCongratulation(true) // Show congratulation animation
      setTimeout(() => {
        setShowCongratulation(false)
        setTransactionState("idle") // Reset transaction state after animation
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl p-8 shadow-lg">
        ...
        <div className="space-y-6">
          ...
          <button onClick={handleMint} disabled={isLoading || transactionState === "pending"} className="btn-23 w-full">
            <span>{transactionState === "pending" ? "Minting..." : "MINT BEUID"}</span>
          </button>
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {ownedUIDs.length > 0 ? (
                ...
                <p className="text-gray-600 dark:text-gray-300 col-span-2">No BlockE UIDs found</p>
              )}
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {showMintedAlert && (
          ...
          >
            ...
            >
              <Image src="/alreadyminted.gif" alt="Already Minted" width={200} height={200} className="mx-auto mb-4" />
      ...
      {showTransactionRejected && <TransactionRejectedMessage onClose={() => setShowTransactionRejected(false)} />}
        ...
        {showCongratulation && (
          ...
          >
            <motion.div className="relative w-96 h-96">
              <Image src="/congratulation.gif" alt="Congratulation" fill className="object-contain" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 text-center relative">
            ...
            <p className="mb-6 text-gray-700">
              You don&apos;t have enough BE tokens to mint this BlockE UID. Please obtain more BE tokens before trying
              again.
            </p>
            <button
              onClick={handleCloseInsufficientBalanceAlert}
      ...
      <TransactionStatus
        isProcessing={transactionState === "pending"}
        isCompleted={transactionState === "success"}
        onClose={() => setTransactionState("idle")}
        message={
          transactionState === "pending"
            ? "Minting BEUID..."
            : transactionState === "success"
              ? "BEUID minted successfully!"
              : transactionState === "error"
                ? "Failed to mint BEUID. Please check error above"
                : ""
        }
